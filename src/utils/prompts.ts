  ...
  return [
    {
      role: 'system' as const,
      content:
        'You are a story lore keeper. Review recent story content and determine if existing lore entries need updating. Always respond with valid JSON only—no prose, no code fences.',
    },
    {
      role: 'user' as const,
      content: `Here are the current lore entries:\n\n${loreBlock}\n\n---\n\nHere is the recent story content:\n\n${recentPassage}\n\n---\n\nDo any lore entries need updating based on new story information (new details, contradictions, new relationships, etc.)?\n\nIf nothing needs updating, respond with exactly: {"updates": false}\n\nIf updates are needed, respond with:\n{\n  "updates": true,\n  "premise": "updated premise text or null",\n  "characters": [\n    {\n      "id": "existing-character-id",\n      "name": "Character Name",\n      "aliases": [],\n      "personality": "...",\n      "appearance": "...",\n      "relationships": [{"id": "rel-id", "targetId": "char-id", "description": "..."}]\n    }\n  ],\n  "locations": [\n    {\n      "id": "existing-location-id",\n      "name": "Location Name",\n      "description": "...",\n      "spatialRelations": [{"id": "rel-id", "targetId": "loc-id", "description": "..."}]\n    }\n  ]\n}\n\nOnly include entries that actually need updating. Preserve all existing IDs exactly.`,
    },
  ];
}
